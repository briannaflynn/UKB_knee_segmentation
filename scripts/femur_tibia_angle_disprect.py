...
def runner(files, path, df):
    ...
    for f in files:
        ...
        if None in [fx2, fy2, tx1, ty1]:
        	...
        	line_thickness = 2
        	cv2.rectangle(image, (fx2, fy2), (tx1, ty1), (0,0,255), thickness=line_thickness)
        	cv2.circle(image, (fx2, fy2), radius=10, color = (0,0,255), thickness=line_thickness)
        	cv2.circle(image, (tx1, ty1), radius=10, color = (0, 0, 255), thickness=line_thickness)
        	cv2.imwrite(f[:-4] + "_disp_rectangle.png", image)
        	print(f[:-4] + "_disp_rectangle.png", "successfully written")
